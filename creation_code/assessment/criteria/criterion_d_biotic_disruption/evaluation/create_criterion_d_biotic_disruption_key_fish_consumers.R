...
##
 ## -- call to fish baseline reference min & max -- ##
  # point to data locale
    data_locale <- "data_raw/biological/fishes/dev/"
...
# # ℹ Use `print(n = ...)` to see more rows
  ...
  # quickview
    fish_data_table %>% quickview()
  # eco_rgn                        site_id first_year recent_year
# 1 Comoros       Mayotte;Douamougno - 3 m       2018        2018
# 2 Comoros       Mayotte;Douamougno = 0 m       2018        2018
# 3 Comoros Mayotte;Double BarriÌ¬re - 3 m       2018        2018
  # current_den recent_den threshold_ref_max threshold_ref_min
# 1         180        180             75.75              3.79
# 2         200        200             75.75              3.79
# 3          60         60             75.75              3.79
     # fish
# 1 grouper
# 2 grouper
# 3 grouper
  ...
  # review threwshold values
    fish_data_table %>%
      group_by(eco_rgn,
               fish) %>%
      reframe(threshold_ref_min = threshold_ref_min %>% mean(na.rm = TRUE),
              threshold_ref_max = threshold_ref_max %>% mean(na.rm = TRUE)) %>%
      arrange(fish)
# # A tibble: 13 × 4
   # eco_rgn             fish  threshold_ref_min threshold_ref_max
   # <chr>               <chr>             <dbl>             <dbl>
 # 1 Comoros             grou…              3.79              75.8
 # 2 Delagoa             grou…             13.9              278. 
 # 3 Mascarene Isl.      grou…              0.77              15.4
 # 4 N Mozambique - S T… grou…              2.53              50.6
 # 5 N Tanzania-Kenya    grou…              1.79              35.7
 # 6 Seychelles north    grou…              1.68              33.6
 # 7 West Madagascar     grou…              2.35              47.0
 # 8 Comoros             parr…              4.93              49.3
 # 9 Delagoa             parr…              5.02             100. 
# 10 N Mozambique - S T… parr…              8.67             173. 
# 11 N Tanzania-Kenya    parr…              6.29              62.9
# 12 Seychelles north    parr…              9.15              91.5
# 13 West Madagascar     parr…              3.1               61.9
